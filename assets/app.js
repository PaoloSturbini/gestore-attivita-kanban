...
const initialData = {
  ...
  ui: {
    ...
    openProjects: seededProjects[0] ? { [seededProjects[0].id]: true } : {},
...
function createTask(id, name, startDate, dueDate, owner, statusId, notes, subtasks, priority = "none", attachments = []) {
  return { id, name, startDate, dueDate, owner, statusId, notes, subtasks, priority, attachments: normalizeAttachments(attachments) };
}
...
function taskOwnersFromProjects(projects) {
  return normalizeParticipants(
    (Array.isArray(projects) ? projects : []).flatMap((project) => (Array.isArray(project.tasks) ? project.tasks.map((task) => task.owner) : [])),
  );
}
...
function sampleWorkspaceProject() {
  return {
    ...
    enabledViews: { board: true, table: true, grid: true, dashboard: true },
...
function normalizeWorkspace(workspace) {
  ...
  return {
    id: workspace?.id || slugify(name || "workspace"),
    name,
    ui,
    projects,
  };
...
function loadState() {
  ...
  sources.sort((a, b) => {
    const updatedDiff = stateUpdatedAtValue(b) - stateUpdatedAtValue(a);
    if (updatedDiff) return updatedDiff;
    const revisionDiff = stateRevisionValue(b) - stateRevisionValue(a);
    if (revisionDiff) return revisionDiff;
    return stateProjectCount(b) - stateProjectCount(a);
  });
...
function normalizeState(nextState) {
  ...
  if (!nextState.projects.some((project) => project.id === nextState.activeProjectId)) {
    nextState.activeProjectId = nextState.projects[0]?.id || "";
  }
  ...
  if (nextState.activeProjectId && typeof nextState.ui.openProjects[nextState.activeProjectId] === "undefined") {
    nextState.ui.openProjects[nextState.activeProjectId] = true;
  }
  ...
  nextState.projects.forEach((project) => {
    ...
    project.tasks.forEach((task) => {
      task.priority = priorityOption(task.priority, true).id;
...
function bindEvents() {
  ...
  els.projectsToggle.addEventListener("click", () => {
    const willOpen = Boolean(state.ui.projectsCollapsed);
    state.ui.projectsCollapsed = !state.ui.projectsCollapsed;
    if (willOpen) openHomePage();
    else {
      saveState();
      renderShell();
    }
...
function renderShell() {
  ...
  els.projectList.innerHTML = state.projects
    .map((item) => {
      ...
      const open = state.ui.openProjects[item.id] !== false;
      return `
  ...
  els.workspaceCreateSlot.innerHTML = workspaceCreateOpen
    ? `
      <form class="inline-project-form workspace-create-form" id="workspaceCreateForm">
        <label>
          Nome spazio
          <input type="text" name="workspaceName" maxlength="70" autocomplete="off" required />
        </label>
...
function renderContent() {
    ...
    els.content.innerHTML = `
      <div class="empty-state">
        <div>
          ...
          Abilita Board, List, Grid o Dashboard dalla Lista progetti.
        </div>
      </div>
    `;
    return;
  ...
  else if (state.activeView === "grid") renderGrid(tasks);
}
...
function renderProjectsOverview() {
  if (!state.projects.length) {
    els.content.innerHTML = `
      <div class="empty-state">
        <div>
          <strong>Nessun progetto ancora.</strong><br />
          Crea il primo progetto per iniziare a organizzare le attività.
        </div>
        ${
          projectCreateOpen
            ...
            <article class="project-summary" tabindex="0">
              ${
                pendingRenameProjectId === project.id
                  ? `
                    <div class="project-summary-main">
                      <form class="inline-project-form rename-project-form" data-project-rename-form="${project.id}">
                        ...
                        <div class="popover-actions">
                          <button type="button" class="secondary-btn" data-cancel-project-rename>Annulla</button>
                  ...
                  `
                  : `
                    <button class="project-summary-main" data-open-project="${project.id}" type="button">
                      <strong>${escapeHtml(project.name)}</strong>
                      ${project.note ? `<span class="project-note-line">${escapeHtml(project.note)}</span>` : ""}
                      <span>${total} attività</span>
                      <span>${done} completate</span>
                      <span>${overdue} scadute</span>
                    </button>
              ...
              <div class="project-summary-actions">
                <button class="secondary-btn" data-note-project="${project.id}" type="button">Nota</button>
    ...
    .map((project) => {
      const tasks = project.tasks.filter((task) => {
        const haystack = [
          ...
          ...task.subtasks.map((item) => `${item.name} ${item.dueDate || ""}`),
        ]
          .join(" ")
          .toLowerCase();
        return (task.owner || "").trim() === owner && (!term || haystack.includes(term));
      });
...
// Sostituiscono i listener riattaccati a ogni card/riga in renderBoard/renderTable/renderGrid.
function bindContentDelegation() {
  els.content.addEventListener("click", (event) => {
    ...
    if (card) {
      ...
      openTaskDialog(card.dataset.task);
    }
  });
...
function renderGrid(tasks) {
  const sortedTasks = [...tasks].sort((a, b) => dueDateSortValue(a) - dueDateSortValue(b) || a.name.localeCompare(b.name));
  els.content.innerHTML = `<div class="grid-view">${sortedTasks.map(taskCard).join("")}</div>`;
  // Click su card: gestito da bindContentDelegation() (delega su #content).
}
...
function tableSubtasks(task) {
  ...
  return `
    <div class="table-subtasks">
      ${task.subtasks
        .map((subtask) => `<span>${subtask.done ? "✓" : "□"} ${escapeHtml(subtask.name)}${subtask.dueDate ? ` · ${formatDate(subtask.dueDate)}` : ""}</span>`)
        .join("")}
    </div>
  `;
}
...
function openTaskDialog(taskId = null, statusId = null) {
  ...
  (task?.subtasks || []).forEach((subtask) => addSubtaskField(subtask.name, subtask.done, subtask.dueDate || "", subtask.id));

  els.taskDialog.showModal();
  els.taskName.focus();
}

function addSubtaskField(name, done, dueDate = "", subtaskId = "") {
  ...
  row.innerHTML = `
    ...
    <input type="text" value="${escapeHtml(name)}" placeholder="Nome sotto-attività" />
...
function saveTaskFromDialog() {
  ...
  const nextTask = {
    ...
    subtasks,
    attachments: normalizeAttachments(taskAttachmentDraft),
  };
...
function deleteTaskFromDialog() {
  const project = activeProject();
  if (!project) return;
  project.tasks = project.tasks.filter((task) => task.id !== els.taskId.value);
  saveState();
  els.taskDialog.close();
  render();
}
...
function exportProjectMarkdown() {
  ...
  const rows = project.tasks.map((task) => {
    ...
    const subtasks = task.subtasks.length
      ? task.subtasks.map((subtask) => `${subtask.done ? "[x]" : "[ ]"} ${subtask.name}${subtask.dueDate ? ` (${formatDate(subtask.dueDate)})` : ""}`).join("<br>")
      : "";
...
function xlsxSheetXml(rows) {
  const editableColumns = new Set([10, 11, 14]);
  const rowXml = rows
    .map(
      (row, rowIndex) => `
        <row r="${rowIndex + 1}">
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  ...
  <autoFilter ref="E1:O${rows.length || 1}" />
  <dataValidations count="1">
    <dataValidation type="list" allowBlank="1" showErrorMessage="1" sqref="N2:N1048576"><formula1>"SI,NO"</formula1></dataValidation>
  </dataValidations>
</worksheet>`;
}
...
function viewName(view) {
  return { board: "Board", table: "List", grid: "Grid" }[view] || "Board";
}
