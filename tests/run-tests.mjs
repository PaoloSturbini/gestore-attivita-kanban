...
assert.match(stylesSource, /project-edit-btn/);
assert.match(stylesSource, /project-actions-popover/);
assert.doesNotMatch(stylesSource, /project-summary-actions/);
assert.match(viewsSource, /project-quick-actions/);
assert.match(viewsSource, /data-project-actions/);
assert.match(viewsSource, /project-actions-popover/);
assert.match(viewsSource, />Elimina<\/button>/);
assert.doesNotMatch(viewsSource, /project-summary-actions/);
assert.match(dialogsSource, /pickTaskAttachment/);
assert.match(dialogsSource, /openAttachmentRemovePopup/);
assert.match(dialogsSource, /data-attachment-remove-action="cancel"/);
assert.match(dialogsSource, /data-attachment-remove-action="remove"/);
assert.match(dialogsSource, /data-attachment-remove-action="trash"/);
assert.match(dialogsSource, /trashTaskAttachment/);
assert.match(dialogsSource, /receiveTaskAttachmentTrashResult/);
...
assert.match(couchInitSource, /portal_users/);
assert.match(couchInitSource, /assignable_names/);
assert.match(couchInitSource, /projects_by_id/);
...
assert.match(portalServerSource, /SECURITY_HEADERS/);
assert.match(portalServerSource, /Content-Security-Policy/);
assert.match(portalServerSource, /X-Content-Type-Options/);
assert.match(portalServerSource, /staticFilePath/);
assert.match(portalServerSource, /relative\(publicDir, filePath\)/);
assert.match(portalServerSource, /portal_users/);
assert.match(portalServerSource, /assignable_names/);
assert.match(portalServerSource, /projects_by_id/);
assert.match(portalServerSource, /couchPostView/);
assert.doesNotMatch(portalServerSource, /_all_docs/);
...
assert.match(stateSource, /remoteSyncQueued/);
assert.match(stateSource, /remoteApplyBlockedByLocalWork/);
assert.match(stateSource, /queueRemoteKanbanApply/);
assert.match(stateSource, /queueRemoteStateDocApply/);
...
assert.match(swiftSource, /trashTaskAttachment/);
assert.match(swiftSource, /trashItem\(at:/);
assert.match(swiftSource, /exportBackupZip/);
assert.match(read("src/09_backup.js"), /directoryPath: state\.ui\.autoBackupDirectoryPath \|\| ""/);
assert.match(read("src/09_backup.js"), /directoryBookmark: state\.ui\.autoBackupDirectoryBookmark \|\| ""/);
assert.match(stateSource, /autoBackupDirectoryBookmark/);
assert.match(swiftSource, /directoryPath: payload\["directoryPath"\] as\? String/);
assert.match(swiftSource, /directoryBookmark: payload\["directoryBookmark"\] as\? String/);
assert.match(swiftSource, /bookmarkData\(options: \[\.withSecurityScope\]/);
assert.match(swiftSource, /startAccessingSecurityScopedResource/);
assert.match(swiftSource, /openBackupSavePanel\(filename: finalFilename/);
assert.match(swiftSource, /KanbanBackup-\\\(UUID\(\)\.uuidString\)\.zip/);
assert.match(swiftSource, /copyItem\(at: tempZip, to: destinationURL\)/);
assert.match(swiftSource, /appendingPathComponent\(finalFilename, isDirectory: false\)/);
